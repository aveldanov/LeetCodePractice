    ...
    func gameOfLife(_ board:  [[Int]]) -> [[Int]]{
        ...
        for i in 0..<column{
            for j in 0..<row {
                ...
                print(helper(&board, i: 0, j: 0, count: &count))
...
//        print("BOOM")
    ...
    func helper(_ board: inout [[Int]], i:Int, j:Int, count:inout Int){
        ...
        helper(&board, i: i+1, j: j, count: &count)
        helper(&board, i: i-1, j: j, count: &count)
        helper(&board, i: i, j: j+1, count: &count)
        helper(&board, i: i, j: j-1, count: &count)
        print(">>>>",count)
